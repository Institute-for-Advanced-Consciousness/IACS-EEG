function mygui_computeica(h,~,data)
cfg=guidata(h);
...
if ishandle(dlg)
    ...
    if param.boxreuse.Value == 0
        ...
        % generate pca timeseries
        
         % Perform PCA, transpose data so rows are observations, columns are variables
        [coeff,score] = pca(alldat'); 
         % retain only the first 24 PCA comps 
        topComps = score(1:24,:);
        coeff = coeff(1:24,:)';
        %Compute ICA from transposed PCs
        [A_prime,W_prime] = fastica(topComps',...
            'approach','defl',...
            'g','pow3',... % ,'tanh'
        ...
        % convert back to sensor space to produce the final a and w
        W = coeff*W_prime; % multiply pca weights by ica weights
        A = W.^-1;
