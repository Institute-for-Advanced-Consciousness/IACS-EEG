...
function [] = scroll_data_alt(cfg,data)
...
cfg.win.tdefault = 15; % 15 sec window (for sleep stage estimation)
...
% done
uicontrol(fig,'units','normalized','position',[0.005 0.005  0.03  0.02],'String','done','Callback',@stop);

% filtering
uicontrol(fig,'units','normalized','position',[0.04 0.005  0.06  0.02],'String','filter','Callback',@mygui_definefiltering);

% toggle montage 
uicontrol(fig,'units','normalized','position',[0.115 0.005  0.06  0.02],'String','clin. montage','Callback',@mygui_togglemontage);
...
% Color sleep related labels brightly, use grayscale for everything else
% (makes sleep labeling easier)
HSV = hsv; % get the hsv colormap
%GS = gray;
chrome = [(HSV(1:40:end,:)./3 + 0.66)];
...
for iaty = 1:length(cfg.dattype.labels)
    cfg.dattype.buttontype{iaty} = uicontrol(fig,'units','normalized','position', ...
        [0.87  0.3+iaty*0.025  0.13  0.025],'Style', 'togglebutton', 'String', cfg.dattype.labels{iaty},...
...
cfg.dattype.boxstart = uicontrol(fig,'Style','edit','units','normalized','position', ...
    [0.87  0.15  0.07  0.05],'String','-','Callback',@start_datasel);
cfg.dattype.boxstop  = uicontrol(fig,'Style','edit','units','normalized','position',...
    [0.94  0.15  0.07  0.05],'String','-','Callback',@stop_datasel);

% EEG channelselection
cfg.disp.guichanselect = uicontrol(fig,'units','normalized','position', ...
    [0.87  0.125  0.13  0.025],'Style', 'pushbutton', 'String', 'channel/comp',...
    'Callback', @channelselect);

% channelplots
uicontrol(fig,'units','normalized','position', ...
    [0.87  0.1  0.13  0.025],'Style', 'pushbutton', 'String', 'c. plots',...
    'Callback', {@mygui_cplots,plotdata});

% computeica
uicontrol(fig,'units','normalized','position', ...
    [0.87  0.075  0.13  0.025],'Style', 'pushbutton', 'String', 'compute ICA',...
    'Callback', {@mygui_computeica,data});

% gamma thresholding
cfg.figlay.boxtopo = uicontrol(fig,'units','normalized','position', ...
    [0.87  0.05  0.13  0.025],'Style', 'pushbutton', 'String', 'set threshold',...
    'Value',0,'Callback',{@mygui_thresh,data});

% ICA overview
cfg.figlay.boxtopo = uicontrol(fig,'units','normalized','position', ...
    [0.87  0.025  0.13  0.025],'Style', 'pushbutton', 'String', 'ICA overview',...
    'Value',0,'Callback',{@mygui_ICA_overview,plotdata});

% showtopos
cfg.figlay.boxtopo = uicontrol(fig,'units','normalized','position', ...
    [0.87  0.00  0.13  0.025],'Style', 'togglebutton', 'String', 'ICA minitopo',...
    'Value',0,'Callback',@ICA_minitopo);

% Display
uicontrol(fig,'units','normalized','position', [0.87  0.89  0.13  0.025],'Style', ...
    'text', 'String', 'Display')
cfg.disp.guiselect = uicontrol(fig,'units','normalized','position', [0.87  0.865  0.13  0.025],'Style', ...
    'popupmenu', 'String', cfg.disp.alllabel,'Value',cfg.disp.icur,...
    'Callback', @selectdisplaymag);

% % Comments
% uicontrol(fig,'units','normalized','position', ...
%     [0.87  0.88  0.13  0.025],'Style', 'pushbutton', 'String', 'Comments',...
%     'Callback', @addcomments);
% cfg.comments = [];

% save
uicontrol(fig,'units','normalized','position', ...
    [0.87  0.915  0.13  0.025],'Style', 'pushbutton', 'String', 'save',...
    'Callback', {@mysave,data});

% Dataout
uicontrol(fig,'units','normalized','position', ...
    [0.87  0.94  0.13  0.025],'Style', 'pushbutton', 'String', 'Output file',...
...
%             end
    ...
    if cfg.redraw.main
        ...
        for i=1:length(idch_alldata)
            plot(cfg.h_main,t,dataplot(idch_alldata(i),:) + i,'Color','k')%auxcolor(i,:))
        end
